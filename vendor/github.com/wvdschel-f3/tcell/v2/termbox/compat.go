...
import (
	...
	"github.com/wvdschel-f3/tcell/v2"
)
...
// GetCell returns the character and attributes at a given location
// from the cell buffer. This offers an alternative to the CellBuffer()
// function from termbox.
func GetCell(x, y int) (ch rune, fg, bg Attribute) {
	ch, _, st, _ := screen.GetContent(x, y)
	fg_, bg_, _ := st.Decompose()
	fg = Attribute(fg_)
	bg = Attribute(bg_)
	return ch, fg, bg
}
